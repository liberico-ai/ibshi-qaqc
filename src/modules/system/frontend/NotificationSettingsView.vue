<template>
  <div class="space-y-6">
    <div>
      <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100">{{ $t('notifications.settings_title') }}</h2>
      <p class="text-xs text-slate-500 mt-1">{{ $t('notifSettings.subtitle') }}</p>
    </div>

    <!-- Channels section -->
    <div class="card p-4 space-y-4">
      <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ $t('notifications.channels') }}</h3>
      ...
      <div v-for="ch in channels" :key="ch.channel_class"
        ...
        <div class="flex-1">
          ...
          <div class="text-xs text-slate-500">
            <span v-if="identityFor(ch.channel_class)?.is_verified" class="text-green-600 dark:text-green-400">
              ✓ {{ $t('notifSettings.connected_short') }} — {{ maskIdentity(identityFor(ch.channel_class)?.identity) }}
            </span>
            <span v-else class="text-slate-400">{{ $t('notifSettings.not_connected_short') }}</span>
          </div>
        </div>
        ...
        <template v-if="ch.channel_class === 'notification-telegram'">
          <button v-if="!identityFor(ch.channel_class)?.is_verified" @click="linkTelegram"
            ...
            {{ $t('notifSettings.connect_telegram') }}
          </button>
        ...
        <template v-else>
          <span class="text-xs text-slate-400">{{ $t('notifSettings.unsupported') }}</span>
        </template>
      </div>
    </div>
    ...
    <div class="card p-0 overflow-hidden">
      <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-800">
        <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ $t('notifications.event_prefs') }}</h3>
      </div>
      <table class="w-full text-sm">
        <thead>
          <tr class="bg-slate-50 dark:bg-[#1a1a2e] border-b border-slate-200 dark:border-slate-800">
            <th class="text-left px-4 py-2 font-semibold text-slate-600 dark:text-slate-400">{{ $t('notifSettings.event') }}</th>
      ...
      <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-800 flex justify-end">
        <button @click="savePrefs" :disabled="saving"
          ...
          {{ saving ? $t('notifSettings.saving') : $t('common.save') }}
        </button>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="telegramModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative z-10 w-full max-w-md bg-white dark:bg-[#1a1a2e] rounded-xl shadow-2xl p-5 space-y-4">
          <h3 class="text-sm font-semibold text-slate-800 dark:text-slate-100">{{ $t('notifSettings.connect_telegram') }}</h3>
          <div class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-sm text-slate-700 dark:text-slate-300 space-y-2">
            <p>{{ $t('notifSettings.tg_step1') }}</p>
            <p>{{ $t('notifSettings.tg_step2') }}</p>
            ...
            <p class="text-xs text-slate-500">{{ $t('notifSettings.tg_expire') }}</p>
          </div>
          <p class="text-xs text-slate-500">{{ telegramModal.instructions }}</p>
          <div class="flex justify-end gap-2">
            <button @click="telegramModal.show = false" class="px-4 py-2 text-sm text-slate-600 dark:text-slate-400">{{ $t('notifSettings.close') }}</button>
            <button @click="reload" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg">
              {{ $t('notifSettings.recheck') }}
            </button>
          </div>
        </div>
      </div>
    ...
    <Teleport to="body">
      <div v-if="mattermostModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative z-10 w-full max-w-md bg-white dark:bg-[#1a1a2e] rounded-xl shadow-2xl p-5 space-y-4">
          <h3 class="text-sm font-semibold text-slate-800 dark:text-slate-100">{{ $t('notifSettings.connect_mattermost') }}</h3>
          <p class="text-xs text-slate-500">{{ $t('notifSettings.mm_hint') }}</p>
          ...
          <div class="flex justify-end gap-2">
            <button @click="mattermostModal.show = false" class="px-4 py-2 text-sm text-slate-600 dark:text-slate-400">{{ $t('common.cancel') }}</button>
            <button @click="saveMattermost" :disabled="mattermostModal.saving || !mattermostModal.url"
              ...
              {{ mattermostModal.saving ? $t('notifSettings.saving') : $t('common.save') }}
            </button>
          </div>
        </div>
      </div>
...
async function unlink(channelClass) {
  if (!confirm(t('notifSettings.unlink_confirm'))) return;
