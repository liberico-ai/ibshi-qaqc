<template>
  <div class="max-w-4xl mx-auto space-y-6">
    <div class="card p-6 border-b border-gray-200 dark:border-[#252540]">
      <h2 class="text-xl font-bold text-slate-800 dark:text-white">{{ $t('profile.account_settings') }}</h2>
      <p class="text-sm text-slate-500 dark:text-gray-400 mt-1">{{ $t('profile.subtitle') }}</p>
    </div>
      ...
      <div class="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div class="flex-1 w-full space-y-5">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label class="block text-[13px] font-semibold text-slate-700 dark:text-gray-300 mb-1.5">{{ $t('profile.full_name') }}</label>
            ...
            <div>
              <label class="block text-[13px] font-semibold text-slate-700 dark:text-gray-300 mb-1.5">{{ $t('profile.username') }}</label>
              <input :value="currentUser?.username" disabled type="text" class="w-full bg-gray-100 dark:bg-[#0a0b0e] border border-gray-200 dark:border-[#252540] rounded-lg px-4 py-2.5 text-sm text-slate-500 dark:text-gray-600 cursor-not-allowed" :placeholder="$t('profile.username')">
              <p class="text-[11px] text-slate-400 dark:text-gray-600 mt-1">{{ $t('profile.username_locked') }}</p>
            </div>
          </div>
          ...
          <div class="pt-2 flex justify-end">
            <button @click="saveProfile" :disabled="savingProfile" class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold px-6 py-2.5 rounded-lg transition-all shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 flex items-center justify-center gap-2 text-[13.5px] disabled:opacity-50">
              ...
              <span>{{ savingProfile ? $t('profile.saving_changes') : $t('profile.save_profile') }}</span>
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="card p-6 md:p-8">
      ...
      <div class="space-y-5 max-w-xl">
        <div class="relative">
          <label class="block text-[13px] font-semibold text-slate-700 dark:text-gray-300 mb-1.5">{{ $t('profile.current_password') }} <span class="text-red-500">*</span></label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            <label class="block text-[13px] font-semibold text-slate-700 dark:text-gray-300 mb-1.5">{{ $t('profile.new_password') }} <span class="text-red-500">*</span></label>
          ...
          <div>
            <label class="block text-[13px] font-semibold text-slate-700 dark:text-gray-300 mb-1.5">{{ $t('profile.confirm_password') }} <span class="text-red-500">*</span></label>
        ...
        <div class="pt-2 flex justify-start">
          <button @click="changePassword" :disabled="savingPassword" class="bg-gradient-to-r from-slate-800 to-slate-900 hover:from-slate-700 hover:to-slate-800 dark:from-white/10 dark:to-white/5 dark:hover:from-white/20 dark:hover:to-white/10 text-white font-semibold px-6 py-2.5 rounded-lg transition-all shadow-lg flex items-center justify-center gap-2 text-[13.5px] disabled:opacity-50">
            ...
            <span>{{ savingPassword ? $t('profile.verifying') : $t('profile.update_password') }}</span>
          </button>
        </div>
      </div>
    </div>
...
const saveProfile = async () => {
  if (!form.value.full_name.trim()) return error(t('profile.name_required'));
  ...
  try {
    ...
    if (res.ok) {
      ...
      error(err.error || t('profile.update_failed'));
    }
  } catch (e) {
    error(t('profile.network_error'));
  } finally {
...
const changePassword = async () => {
  if (!pass.value.current || !pass.value.new || !pass.value.confirm) return error(t('profile.password_fields_required'));
  if (pass.value.new !== pass.value.confirm) return error(t('profile.password_mismatch'));
  ...
  try {
    ...
    if (res.ok) {
      success(t('profile.password_updated'));
      pass.value = { current: '', new: '', confirm: '' };
    } else {
      const err = await res.json();
      error(err.error || t('profile.password_change_failed'));
    }
  } catch (e) {
    error(t('profile.network_error'));
  } finally {
