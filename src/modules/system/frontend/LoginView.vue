    ...
    <div class="w-full max-w-md p-8 bg-white dark:bg-[#1a1a2e] rounded-2xl shadow-2xl border border-gray-200 dark:border-[#252540] relative z-10 mx-4">
      ...
      <form @submit.prevent="handleLogin" class="space-y-5">
        <div>
          ...
          <input v-model="form.username" required type="text" class="w-full bg-slate-50 dark:bg-[#0f1117] border border-gray-200 dark:border-[#252540] rounded-lg px-4 py-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:bg-white focus:border-blue-500 dark:focus:border-blue-500 dark:focus:bg-[#151521] shadow-sm transition-all" placeholder="Enter your username">
        </div>
        <div>
          <label class="block text-[13px] font-semibold text-slate-700 dark:text-gray-300 mb-1.5">Password</label>
          <input v-model="form.password" required type="password" class="w-full bg-slate-50 dark:bg-[#0f1117] border border-gray-200 dark:border-[#252540] rounded-lg px-4 py-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:bg-white focus:border-blue-500 dark:focus:border-blue-500 dark:focus:bg-[#151521] shadow-sm transition-all" placeholder="••••••••">
        </div>

        <div v-if="errorMessage" class="text-[13px] font-medium text-red-500 bg-red-50 dark:bg-red-500/10 p-3 rounded-lg border border-red-100 dark:border-red-500/20 text-center flex items-center justify-center gap-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          {{ errorMessage }}
        </div>

        <button :disabled="loading" type="submit" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold py-3 flex items-center justify-center gap-2 rounded-lg transition-all shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 mt-4 disabled:opacity-50">
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      errorMessage.value = err.error || 'Login failed';
    }
  } catch (e) {
    errorMessage.value = 'Network error connecting to server';
  } finally {
    loading.value = false;
  }
};
