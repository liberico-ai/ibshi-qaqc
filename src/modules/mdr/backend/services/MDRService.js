...
export class MDRService {
  ...
  /**
   * Biên dịch dossier thành 1 file PDF gộp (bìa + mục lục).
   * Nếu file nguồn không có sẵn, sinh PDF placeholder. Trả về đường dẫn temp.
   */
  static async compileBundle(dossierId) {
    ...
    // ── Table of contents ──
    const toc = pdfDoc.addPage([595, 842]);
    ...
    for (const cat of detail.categories) {
      ...
      const present = docsInCat.filter((d) => d.status === 'present').length;
      const mark = cat.required ? (present > 0 ? '[OK]' : '[MISSING]') : '[OPT]';
      toc.drawText(
        `${String(cat.order_no).padStart(2, '0')}. ${this._ascii(cat.name_vi)}  ${mark}  (${present}/${docsInCat.length})`,
        { x: 60, y, size: 11, font }
      );
      y -= 22;
      if (y < 60) { y = 780; pdfDoc.addPage([595, 842]); }
    }

    const bytes = await pdfDoc.save();
    const outPath = path.join(os.tmpdir(), `mdr_bundle_${dossierId}_${Date.now()}.pdf`);
    await fsp.writeFile(outPath, bytes);
    log.info({ dossierId, outPath }, 'MDR bundle compiled');
    return { path: outPath, pages: pdfDoc.getPageCount(), note: 'PDF gồm bìa + mục lục (placeholder cho file nguồn chưa có)' };
  }
  ...
  static async buildSubmission(dossierId, client) {
    ...
    const manifest = {
      ...
      categories: detail.categories.map((c) => ({
        ...
        documents: detail.documents
          ...
          .map((d) => ({ doc_name: d.doc_name, status: d.status, file_link: d.file_link, source_module: d.source_module })),
      })),
    };
    archive.append(JSON.stringify(manifest, null, 2), { name: 'manifest.json' });

    // Document placeholders / available source links
    for (const doc of detail.documents) {
      ...
      if (doc.status === 'present' && doc.file_link) {
        archive.append(`Tai lieu: ${doc.doc_name}\nNguon: ${doc.file_link}\nModule: ${doc.source_module ?? '-'}\n`,
          { name: `${folder}/${this._safe(doc.doc_name)}.txt` });
      } else {
        archive.append(`THIEU TAI LIEU (placeholder): ${doc.doc_name}\nTrang thai: ${doc.status}\n`,
    ...
    log.info({ dossierId, outPath }, 'MDR submission built');
    return { path: outPath, transmittal_no: transNo, doc_count: detail.documents.length };
  }
