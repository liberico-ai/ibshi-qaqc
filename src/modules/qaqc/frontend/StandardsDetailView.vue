  ...
  <div class="space-y-4">
    ...
    <template v-else-if="standard">
      ...
      <div class="card p-4 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
        <div><span class="block text-xs text-slate-500 mb-1">Nhóm</span><span class="font-medium text-slate-800 dark:text-slate-200">{{ standard.grp ?? '—' }}</span></div>
        <div><span class="block text-xs text-slate-500 mb-1">Phiên bản</span><span class="font-medium text-slate-800 dark:text-slate-200">{{ standard.version ?? '—' }}</span></div>
        <div><span class="block text-xs text-slate-500 mb-1">Ban hành</span><span class="font-medium text-slate-800 dark:text-slate-200">{{ standard.issued_date ?? '—' }}</span></div>
        <div><span class="block text-xs text-slate-500 mb-1">Trạng thái</span>
      ...
      <div v-if="standard.specs?.chemical?.length" class="card p-0 overflow-hidden">
        <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-700 font-semibold text-sm text-slate-700 dark:text-slate-300">Thành phần hóa học</div>
        <table class="w-full text-sm">
          <thead><tr class="bg-slate-50 dark:bg-[#1a1a2e]">
            <th class="px-4 py-2 text-left text-slate-600 dark:text-slate-400">Mác thép</th>
            <th class="px-4 py-2 text-left text-slate-600 dark:text-slate-400">Nguyên tố</th>
            <th class="px-4 py-2 text-right text-slate-600 dark:text-slate-400">Min</th>
            <th class="px-4 py-2 text-right text-slate-600 dark:text-slate-400">Max</th>
            <th class="px-4 py-2 text-left text-slate-600 dark:text-slate-400">Đơn vị</th>
          </tr></thead>
      ...
      <div v-if="standard.specs?.mechanical?.length" class="card p-0 overflow-hidden">
        <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-700 font-semibold text-sm text-slate-700 dark:text-slate-300">Cơ tính</div>
        <table class="w-full text-sm">
          <thead><tr class="bg-slate-50 dark:bg-[#1a1a2e]">
            <th class="px-4 py-2 text-left text-slate-600 dark:text-slate-400">Mác thép</th>
            <th class="px-4 py-2 text-left text-slate-600 dark:text-slate-400">Thuộc tính</th>
            <th class="px-4 py-2 text-right text-slate-600 dark:text-slate-400">Min</th>
            <th class="px-4 py-2 text-right text-slate-600 dark:text-slate-400">Max</th>
            <th class="px-4 py-2 text-left text-slate-600 dark:text-slate-400">Đơn vị</th>
          </tr></thead>
